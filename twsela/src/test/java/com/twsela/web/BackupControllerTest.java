...
@WebMvcTest(controllers = BackupController.class, properties = {
        "app.security.jwt.secret=dGVzdC1zZWNyZXQta2V5LWZvci1qd3QtdG9rZW4tbXVzdC1iZS1sb25n",
        "app.security.jwt.expiration-ms=3600000"
})
@Import(BackupControllerTest.TestMethodSecurityConfig.class)
class BackupControllerTest {
    ...
    @Test
    @DisplayName("GET /api/backup/status — يجب إرجاع حالة النسخ الاحتياطي بنجاح")
    void getBackupStatus_success() throws Exception {
        when(backupService.getBackupStatus()).thenReturn("OK");

        mockMvc.perform(get("/api/backup/status").with(authentication(createAuth("OWNER"))))
    ...
    @Test
    @DisplayName("POST /api/backup/create — يجب إنشاء نسخة احتياطية بنجاح")
    void createBackup_success() throws Exception {
        when(backupService.createBackup()).thenReturn(true);
    ...
    @Test
    @DisplayName("POST /api/backup/create — يجب رفض الوصول للمندوب")
    void createBackup_forbidden_forCourier() throws Exception {
        mockMvc.perform(post("/api/backup/create")
