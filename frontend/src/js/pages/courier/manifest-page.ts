...
import { Shipment, ShipmentStatus, ShipmentItem } from '../../types';
...
// Extended Shipment type with courier-specific fields
interface CourierShipment extends Omit<Shipment, 'id'> {
  id: number;
...
export default class CourierManifestPage extends BasePage {
  ...
  private shipments: CourierShipment[] = [];
  ...
  protected async loadData(): Promise<void> {
    try {
      ...
      // Cast to CourierShipment since we know the response includes the required fields
      this.shipments = response as CourierShipment[];
      this.calculateStats();
      this.optimizeRoute();
      this.renderManifest();
    } catch (error) {
    ...
    this.manifestContainer.addEventListener('click', async (e) => {
      ...
      try {
        switch (action) {
          ...
          case 'return':
            const reason = await this.confirmAction('Please enter the return reason:');
            if (reason) {
  ...
  private renderShipmentCard(shipment: CourierShipment, index: number): string {
    return `
      <div class="manifest-item" data-shipment-id="${shipment.id.toString()}">
        <div class="manifest-header">
  ...
  private renderActionButtons(shipment: CourierShipment): string {
  ...
  private async updateStatus(
    ...
    try {
      ...
      // Update local data
      this.shipments = this.shipments.filter(s => s.id.toString() !== shipmentId);
      this.calculateStats();
      this.renderManifest();
  ...
  private optimizeRoute(): void {
    ...
    const zoneGroups = this.shipments.reduce((groups, shipment) => {
      ...
      return groups;
    }, {} as Record<number, CourierShipment[]>);
